...
const unsigned char APPLEPAY [] = {0x41, 0x70, 0x70, 0x6c, 0x65, 0x50, 0x61, 0x79};
...
static int ParseSelectVASResponse(const uint8_t *response, size_t resLen, bool verbose) { // Interpret the response from sending the aid
    ...
    const struct tlvdb *applicationTLV = tlvdb_find_full(tlvRoot, 0x50); // Find Mobile Application Version, 0x50 = 80
